...
const CryptoDetails = () => {
  ...
  const stats = [
    ...
    { title: '24h Volume', value: `$ ${cryptoDetails && cryptoDetails.volume ? millify(cryptoDetails.volume) : ''}`, icon: <ThunderboltOutlined /> },
    { title: 'Market Cap', value: `$ ${cryptoDetails && cryptoDetails.marketCap && millify(cryptoDetails.marketCap)}`, icon: <DollarCircleOutlined /> },
    { title: 'All-time-high(daily avg.)', value: `$ ${cryptoDetails && cryptoDetails.allTimeHigh && cryptoDetails.allTimeHigh.price && millify(cryptoDetails.allTimeHigh.price)}`, icon: <TrophyOutlined /> },
  ];
  ...
  const genericStats = [
    ...
    { title: 'Aprroved Supply', value: cryptoDetails && cryptoDetails.supply && cryptoDetails.supply.confirmed ? <CheckOutlined /> : <StopOutlined />, icon: <ExclamationCircleOutlined /> },
    { title: 'Total Supply', value: `$ ${cryptoDetails && cryptoDetails.supply && cryptoDetails.supply.total && millify(cryptoDetails.supply.total)}`, icon: <ExclamationCircleOutlined /> },
    { title: 'Circulating Supply', value: `$ ${cryptoDetails && cryptoDetails.supply && cryptoDetails.supply.circulating && millify(cryptoDetails.supply.circulating)}`, icon: <ExclamationCircleOutlined /> },
  ];
